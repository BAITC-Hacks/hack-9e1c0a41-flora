...
- вопросы о выгоде абонента не отправляются в LLM: панель отвечает фиксированным фактом о модели
  (выгода абонента не моделируется) — модель склонна додумывать здесь даже при явном запрете;
- каждое число в ответе сверяется с переданными данными (допуск 1 %, запись в тыс./млн);
  ответ с неподтверждённым числом отклоняется, показывается локальное объяснение.
