...
Финальная версия алгоритма: **v5**, логика зафиксирована в коммите `e1a62ee`.
...
```text
История + профиль аудитории + справочники
                    |
             agent.py / Agent
       гипотезы -> выбор пилота
                    |
             env.run_pilot()
                    |
       наблюдение -> обновление модели
                    |
      MILP / запасной жадный отбор
                    |
            финальные кампании
                    |
     scoring_core.py / общий скорер
                    |
   local_eval.py -> метрики, журнал, план
                    |
                 app.py

make_submission.py -> submission.csv
stress_eval.py -> синтетические эффекты + сравнение с оракулом
```
