...
- Необязательный ассистент по готовому плану, только в панели. Без ключа или
  при ошибке API показывает локальное объяснение; решения агента не меняет.
...
Ключ берётся из `OPENAI_API_KEY` в окружении процесса, модель из `OPENAI_MODEL`
(по умолчанию `gpt-4.1-mini`). `.env` автоматически не читается.
